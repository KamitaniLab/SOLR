function [ model ] = updateBeta_v4_withSampleImportance(X,y,model,weight4sample)
...
func4fminunc=@(beta)func2minimize(X(:,model.effectiveDim),y,model,beta,weight4sample);
options = optimoptions('fminunc','Algorithm','trust-region',...
    'Display','off','SpecifyObjectiveGradient',true,'HessianFcn','objective','MaxIterations',5);
...
function [funcValue gradValue hessValue] = func2minimize(X,y,model,betaAsArgument,weight4sample)
    ...
    for index_sample=1:size(y,1)
        ...
        gradValue=gradValue+(weight4sample(index_sample).*dlogP_dbeta).*(-X(index_sample,:)');
    end
    %add ARD prior
    ...
    for index_sample=1:size(y,1)
        ...
        hessValue=hessValue+(weight4sample(index_sample).*d2logP_dbetabeta).*((X(index_sample,:)')*X(index_sample,:));
    end
    %add ARD prior
