function [ model ] = updateMu_v6(X,y,model)
...
%   Detailed explanation goes here
    ...
    %options = optimset('Algorithm','sqp','Hessian','off','GradObj','on','Display','off','MaxIterations',10);
    %options = optimoptions('fmincon','Algorithm','sqp','Hessian','off','GradObj','on','Display','off','MaxIterations',10);
    options = optimoptions('fmincon','Algorithm','sqp','Hessian','off',...
        'GradObj','on','Display','iter','MaxIterations',10);
...
function [funcValue gradValue] = func2minimize(X,y,model,muAsArgument)
